...
`dim_analyst`, `dim_service`, `dim_modality`, `dim_provider`, `dim_channel`, `dim_sector`.
...
|`dim_provider` (planejado — dimensão ainda não iniciada)|tabela de preços, regiões de cobertura, especialidades, status (intenção registrada na Referência, a confirmar quando a dimensão for aberta)|telefone, e-mail, contato|
